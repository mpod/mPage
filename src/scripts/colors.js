...
let Colors = function(config, model) {
  this.model = model;
  this.config = Utils.extend({dark: {}, light: {}}, config);
  this.dirty = false;
}
...
Colors.prototype = {
  ...
  getShuffledScheme: function(type) {
    var h, H, backS, darkL, backL, i, minS, maxS, minL, maxL, s, l, rangeL;
    var colors = [];
    ...
    for (i=0; i<8; i++) {
      colors.push(Utils.husl.toHex(H[0], backS, backL + rangeL * Math.pow(i/7, 1.5)));
    }
    ...
    for (i=0; i<8; i++) {
      ...
      s = random(minS, maxS);
      l = random(minL, maxL);
      colors.push(Utils.husl.toHex(h, s, l));
    }
